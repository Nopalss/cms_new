...
var KTDatatableLocalSortDemo = function () {
    ...
    // Menghindari race condition dari multiple concurrent reload
    ...
    var demo = function () {
        ...
        datatable = $('#kt_datatable').KTDatatable({
            data: {
                ...
                source: {
                    read: {
                        url: HOST_URL + 'api/request_maintenance.php',
                        params: {
                            query: {
            ...
            columns: [{
                ...
                title: 'RM Id',
                autoHide: false,
                template: row => `<span style="font-size:0.885rem">${row.rm_id}</span>`
            }, {
                field: 'netpay_id',
                title: 'Netpay Id',
                autoHide: false,
                ...
                field: 'type_issue',
                title: 'Type Issue',
                template: row => `<span style="font-size:0.885rem">${row.type_issue}</span>`
            }, {
                field: 'request_by',
                title: 'Request By',
                ...
                template: function (row) {
                    ...
                    if (row.status === 'Pending') {
                        editDelete = `
                        <a href='${HOST_URL + 'pages/request/maintenance/update.php?id=' + row.rm_id}'
                           class="btn btn-sm btn-warning btn-icon mr-2">
                            <i class="flaticon-edit"></i>
                        </a>
                        <a onclick="confirmDeleteTemplate('${row.rm_key}', 'controllers/request/maintenance/delete.php')"
                    ...
                    return `
                    <a data-rm-id="${row.rm_id}" data-status="${row.status}" data-state="${statusMap[current].state}"
                       data-netpay-id="${row.netpay_id}" data-type="${row.type_issue}"
                       data-deskripsi="${row.deskripsi_issue}" data-request-by="${row.request_by}"
                       class="btn btn-sm btn-info btn-icon btn-detail-rm mr-2">
