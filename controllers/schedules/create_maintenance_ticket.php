...
try {
    ...
    $stmt = $pdo->prepare("
        ...
        VALUES (:netpay_id, :queue_id, 'Maintenance', 'Accepted')
    ");
    $stmt->execute([
        ':netpay_id' => $netpay_id,
        ':queue_id'  => $queue_id
    ...
    $stmt = $pdo->prepare("
        INSERT INTO schedules
            (schedule_id, tim_id, date, time, job_type, status, target_status, queue_id, noc_id)
        VALUES
            (:schedule_id, :tim_id, :date, :time, 'Maintenance', 'Pending', 'On Time', :queue_id, :noc_id)
    ");
    $stmt->execute([
