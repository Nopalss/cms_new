...
try {
    $today = date('Y-m-d');
    $sumStmt = $pdo->prepare(
        "SELECT COUNT(DISTINCT sr.srv_id) AS total
         FROM service_reports sr
         LEFT JOIN service_report_pic srp ON sr.srv_id = srp.srv_id
         WHERE DATE(sr.created_at) = :today"
    );
    ...
    if ($_SESSION['role'] == 'teknisi') {
        $sumStmt = $pdo->prepare(
            "SELECT COUNT(DISTINCT sr.srv_id) AS total
             FROM service_reports sr
             LEFT JOIN service_report_pic srp ON sr.srv_id = srp.srv_id
             WHERE DATE(sr.created_at) = :today AND FIND_IN_SET(:tech_id, srp.tech_id)"
        );
        ...
        <div class="container">
            ...
            <div class="registrasi-page-header">
                <div>
                    ...
                    <p>Kelola data laporan perbaikan/service pelanggan.</p>
                </div>
                <div class="header-actions">
                    <!-- <a href="<?= BASE_URL ?>pages/service_report/export_excel.php" class="btn btn-outline-success font-weight-bolder">
            ...
            <div class="card card-custom">
                <div class="card-header flex-wrap border-0 pt-6 pb-0">
                    <div class="card-title">
                        <h3 class="card-label">Data Service Reports</h3>
                    </div>
                </div>
                <div class="card-body">
                    <!--begin::Search Form-->
