...
$stmt = $pdo->prepare($q);
$stmt->execute();
$type_issue = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
try {
    ...
    $sql = "SELECT rm.*, c.*
            ...
            JOIN queue_scheduling q ON rm.queue_id = q.queue_id
...
$standardIssues = array_map(function($t) { return $t['catatan']; }, $type_issue);
$isCustomIssue = !empty($row['type_issue']) && !in_array($row['type_issue'], $standardIssues);
...
<style>
    /* ── Minimal custom overrides (sama dengan create.php) ── */
    :root {
        --primary: #3B82F6;
        --primary-dark: #1D4ED8;
        --navy: #0F172A;
    ...
    <div class="d-flex flex-column-fluid py-8">
        <div class="container">
            ...
            <div class="mb-6 d-flex align-items-center justify-content-between flex-wrap">
                <div>
                    <div class="page-heading">Update Request Servis</div>
                    <div class="page-sub mt-1">Edit detail tiket maintenance yang sudah ada</div>
                </div>
                <a href="<?= BASE_URL ?>pages/request/maintenance" class="btn-rs-cancel mt-2 mt-md-0">
                    <i class="flaticon2-left-arrow-1 mr-1" style="font-size:.75rem;"></i> Kembali
                </a>
            </div>

            <form method="post" action="<?= BASE_URL ?>controllers/request/maintenance/update.php">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                 ...
                                 <div class="form-group">
                                     ...
                                     <select class="form-control rs-select" id="type_issue_select" <?= !$isCustomIssue ? 'name="type_issue"' : '' ?> style="height:44px;" required>
                                         <option value="">— Pilih Jenis Masalah —</option>
                                         <?php foreach ($type_issue as $t): ?>
                                             <?php $selected = ($t['catatan'] == $row['type_issue']) ? 'selected' : '' ?>
                                             <option value="<?= htmlspecialchars($t['catatan']) ?>" <?= $selected ?>>
                                                 <?= htmlspecialchars($t['catatan']) ?>
                                             </option>
                                         <?php endforeach; ?>
                                         <option value="Lainnya" <?= $isCustomIssue ? 'selected' : '' ?>>Lainnya</option>
                                     </select>
                                     <div class="form-group mt-3" id="type_issue_lainnya_group" style="<?= $isCustomIssue ? '' : 'display:none;' ?>">
                                         <label class="font-weight-bold text-dark-50 font-size-sm">Masalah Lainnya</label>
                                         <input type="text" class="form-control rs-input" id="type_issue_lainnya" <?= $isCustomIssue ? 'name="type_issue"' : '' ?> value="<?= $isCustomIssue ? htmlspecialchars($row['type_issue']) : '' ?>" placeholder="Masukkan jenis masalah lainnya" style="height:44px;">
                                     </div>
                                 </div>
                                ...
                                <div class="form-group mb-0">
                                    ...
                                    <textarea
                                        ...
                                        rows="3"
                                        placeholder="Jelaskan masalah yang dialami customer..."><?= htmlspecialchars($row['deskripsi_issue'] ?? '') ?></textarea>
                                </div>
