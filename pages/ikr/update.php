...
try {
    ...
    // ── Ambil data laporan IKR + data customer + tech_id jadwal ───────
    $stmt = $pdo->prepare("
        SELECT 
            ikr.*, 
    ...
    // ── PIC Teknisi: tim / single sesuai tech_id jadwal ──────────────
    $scheduleTechId = $row['schedule_tech_id'] ?? null;
    $isTim          = !empty($scheduleTechId) && strpos($scheduleTechId, 'TIM') === 0;
    $teamMembers    = [];

    if ($isTim) {
        $q = $pdo->prepare("SELECT tech_id, name FROM technician WHERE tim_id = :id");
        $q->execute([':id' => $scheduleTechId]);
        $teamMembers = $q->fetchAll(PDO::FETCH_ASSOC);
    } elseif (!empty($scheduleTechId)) {
    ...
    // ── PIC yang sudah tersimpan di laporan ini (untuk prefill) ──────
    $q = $pdo->prepare("
        SELECT irp.tech_id, t.name
    ...
    $currentPicIds = array_column($currentPics, 'tech_id');

    // Gabungkan sumber nama teknisi (tim + pic tersimpan) untuk mapping JS
    $technicianMapSource = $teamMembers;
    foreach ($currentPics as $cp) {
        $found = false;
        foreach ($technicianMapSource as $tm) {
            if ($tm['tech_id'] === $cp['tech_id']) {
                $found = true;
                break;
            }
        }
        if (!$found) $technicianMapSource[] = $cp;
    }
...
// ── Build variables ──────────────────────────────────────────────
$cust_address = trim(($row['perumahan'] ?? '') . ' ' . ($row['location'] ?? ''));

// Helper kecil: cek apakah value tersimpan ada di daftar dropdown
$normalize = function ($v) {
    return strtoupper(trim((string)$v));
};

$desaList = array_column($desa, 'desa');
$kecList  = array_column($kec, 'kecamatan');
$kabList  = array_column($kab, 'kabupaten');
$typeOntList = array_column($listTypeOnt, 'type_ont');

$desaVal      = $row['desa'] ?? '';
$kecVal       = $row['kec']  ?? '';
$kabVal       = $row['kab']  ?? '';
$typeOntVal   = $row['type_ont'] ?? '';

$desaIsCustom    = $desaVal !== '' && !in_array($normalize($desaVal), $desaList, true);
$kecIsCustom     = $kecVal  !== '' && !in_array($normalize($kecVal), $kecList, true);
$kabIsCustom     = $kabVal  !== '' && !in_array($normalize($kabVal), $kabList, true);
$typeOntIsCustom = $typeOntVal !== '' && !in_array($normalize($typeOntVal), $typeOntList, true);
...
<div class="content">
    <div class="container px-3 pb-5">
        ...
        <form id="formReport" method="post"
            action="<?= BASE_URL ?>controllers/report/ikr/update.php"
            novalidate>

            <input type="hidden" name="ikr_id" value="<?= htmlspecialchars($row['ikr_id']) ?>">
            ...
            <div id="captureForm">
                ...
                <div class="card sr-card shadow-sm mt-3 mb-3">
                    ...
                    <div class="card-body py-3">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            ...
                            <small class="text-muted font-weight-bold"><?= htmlspecialchars($row['ikr_id']) ?></small>
                        </div>
                        <h5 class="font-weight-bolder mb-2" style="color:#0F172A;letter-spacing:-.3px">
                            <?= htmlspecialchars($row['name']) ?>
                        </h5>
                        <div class="d-flex flex-wrap" style="gap:6px">
                            <span class="badge badge-light py-2 px-3" readonly style="border-radius:20px;font-size:12px">
                                📞 <?= htmlspecialchars($row['phone']) ?>
                            </span>
                            <span class="badge badge-light py-2 px-3" style="border-radius:20px;font-size:12px">
                                📶 <?= htmlspecialchars($row['paket_internet']) ?> Mbps
                            </span>
                            <span class="badge badge-light py-2 px-3" style="border-radius:20px;font-size:12px">
                                📍 <?= htmlspecialchars($row['alamat'] ?: ($cust_address ?: '-')) ?>
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="card sr-card shadow-sm mb-3">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input class="form-control sr-control" value="<?= htmlspecialchars($row['netpay_id'] ?? '') ?>" readonly>
                        </div>
                        <div class="form-group">
                            <label class="sr-label">Nama (IKR An) <span class="text-danger">*</span></label>
                            <input class="form-control sr-control validate-field" name="ikr_an" value="<?= htmlspecialchars($row['name'] ?: ($row['name'] ?? '')) ?>" placeholder="Nama pelanggan" readonly>
                            <div class="invalid-feedback">⚠️ Nama wajib diisi</div>
                        </div>
                        <div class="form-group">
                            <label class="sr-label">Telepon <span class="text-danger">*</span></label>
                            <input class="form-control sr-control validate-field" name="telp" pattern="^08[0-9]{8,11}$" value="<?= htmlspecialchars($row['phone']) ?>" placeholder="08xxxxxxxxxx">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control sr-control validate-field" name="alamat" value="<?= htmlspecialchars($row['alamat'] ?? '') ?>" placeholder="Alamat lengkap">
                            <div class="invalid-feedback">⚠️ Alamat wajib diisi</div>
                        </div>
                        <div class="form-row mb-0">
                            <div class="col-6">
                                <label class="sr-label">RT <span class="text-danger">*</span></label>
                                <input class="form-control sr-control validate-field" type="number" name="rt" value="<?= htmlspecialchars($row['rt'] ?? '') ?>" placeholder="RT">
                                <div class="invalid-feedback">⚠️ RT wajib diisi</div>
                            </div>
                            <div class="col-6">
                                <label class="sr-label">RW <span class="text-danger">*</span></label>
                                <input class="form-control sr-control validate-field" type="number" name="rw" value="<?= htmlspecialchars($row['rw'] ?? '') ?>" placeholder="RW">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <select class="form-control sr-control validate-field selectpicker" id="desa" <?= $desaIsCustom ? '' : 'name="desa"' ?> data-size="7" data-live-search="true">
                                <option value="">— Pilih desa —</option>
                                <option value="LAINNYA" <?= $desaIsCustom ? 'selected' : '' ?>>Lainnya</option>
                                <?php foreach ($desa as $p): ?>
                                    <option value="<?= htmlspecialchars($p['desa']) ?>" <?= (!$desaIsCustom && $normalize($desaVal) === $p['desa']) ? 'selected' : '' ?>><?= htmlspecialchars($p['desa']) ?></option>
                                <?php endforeach; ?>
                            </select>
                            <div class="invalid-feedback">⚠️ Desa wajib dipilih</div>
                            <div class="form-group mt-3" id="desa_lainnya_group" style="display:<?= $desaIsCustom ? 'block' : 'none' ?>;">
                                <label class="sr-label">Nama Desa Lainnya</label>
                                <input type="text" class="form-control sr-control validate-field" id="desa_lainnya" <?= $desaIsCustom ? 'name="desa"' : '' ?> value="<?= $desaIsCustom ? htmlspecialchars($desaVal) : '' ?>" placeholder="Masukkan nama desa">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control sr-control validate-field selectpicker" id="kec" <?= $kecIsCustom ? '' : 'name="kec"' ?> data-size="7" data-live-search="true">
                                <option value="">— Pilih Kecamatan —</option>
                                <option value="LAINNYA" <?= $kecIsCustom ? 'selected' : '' ?>>Lainnya</option>
                                <?php foreach ($kec as $p): ?>
                                    <option value="<?= htmlspecialchars($p['kecamatan']) ?>" <?= (!$kecIsCustom && $normalize($kecVal) === $p['kecamatan']) ? 'selected' : '' ?>><?= htmlspecialchars($p['kecamatan']) ?></option>
                                <?php endforeach; ?>
                            </select>
                            <div class="invalid-feedback">⚠️ Kecamatan wajib dipilih</div>
                            <div class="form-group mt-3" id="kec_lainnya_group" style="display:<?= $kecIsCustom ? 'block' : 'none' ?>;">
                                <label class="sr-label">Nama Kecamatan Lainnya</label>
                                <input type="text" class="form-control sr-control validate-field" id="kec_lainnya" <?= $kecIsCustom ? 'name="kec"' : '' ?> value="<?= $kecIsCustom ? htmlspecialchars($kecVal) : '' ?>" placeholder="Masukkan nama kecamatan">
                        ...
                        <div class="form-group mb-0">
                            ...
                            <select class="form-control sr-control validate-field selectpicker" id="kab" <?= $kabIsCustom ? '' : 'name="kab"' ?> data-size="7" data-live-search="true">
                                <option value="">— Pilih Kabupaten —</option>
                                <option value="LAINNYA" <?= $kabIsCustom ? 'selected' : '' ?>>Lainnya</option>
                                <?php foreach ($kab as $p): ?>
                                    <option value="<?= htmlspecialchars($p['kabupaten']) ?>" <?= (!$kabIsCustom && $normalize($kabVal) === $p['kabupaten']) ? 'selected' : '' ?>><?= htmlspecialchars($p['kabupaten']) ?></option>
                                <?php endforeach; ?>
                            </select>
                            <div class="invalid-feedback">⚠️ Kabupaten wajib dipilih</div>
                            <div class="form-group mt-3" id="kab_lainnya_group" style="display:<?= $kabIsCustom ? 'block' : 'none' ?>;">
                                <label class="sr-label">Nama Kabupaten Lainnya</label>
                                <input type="text" class="form-control sr-control validate-field" id="kab_lainnya" <?= $kabIsCustom ? 'name="kab"' : '' ?> value="<?= $kabIsCustom ? htmlspecialchars($kabVal) : '' ?>" placeholder="Masukkan nama Kabupaten">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <select class="form-control sr-control validate-field selectpicker" id="type_ont" <?= $typeOntIsCustom ? '' : 'name="type_ont"' ?> data-size="7" data-live-search="true">
                                <option value="">— Pilih Type ONT —</option>
                                <option value="LAINNYA" <?= $typeOntIsCustom ? 'selected' : '' ?>>Lainnya</option>
                                <?php foreach ($listTypeOnt as $ont): ?>
                                    <option value="<?= htmlspecialchars($ont['type_ont']) ?>" <?= (!$typeOntIsCustom && $normalize($typeOntVal) === $ont['type_ont']) ? 'selected' : '' ?>><?= htmlspecialchars($ont['type_ont']) ?></option>
                                <?php endforeach; ?>
                            </select>
                            <div class="invalid-feedback">⚠️ Type ONT wajib dipilih</div>
                            <div class="form-group mt-3" id="type_ont_lainnya_group" style="display:<?= $typeOntIsCustom ? 'block' : 'none' ?>;">
                                <label class="sr-label">Nama Type ONT Lainnya</label>
                                <input type="text" class="form-control sr-control validate-field" id="type_ont_lainnya" <?= $typeOntIsCustom ? 'name="type_ont"' : '' ?> value="<?= $typeOntIsCustom ? htmlspecialchars($typeOntVal) : '' ?>" placeholder="Masukkan type ONT">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control sr-control validate-field" name="sn" value="<?= htmlspecialchars($row['sn'] ?? '') ?>" placeholder="Serial number">
                        ...
                        <div class="form-group mb-0">
                            ...
                            <input class="form-control sr-control validate-field" name="redaman" value="<?= htmlspecialchars($row['redaman'] ?? '') ?>" placeholder="-xx dBm">
                    ...
                    <div class="card-body">
                        <div class="form-row mb-3">
                            <div class="col-4">
                                ...
                                <input class="form-control sr-control validate-field" name="odp_no" value="<?= htmlspecialchars($row['odp_no'] ?? '') ?>" placeholder="ODP No">
                                <div class="invalid-feedback">⚠️ Wajib diisi</div>
                            </div>
                            <div class="col-4">
                                <label class="sr-label">ODC No <span class="text-danger">*</span></label>
                                <input class="form-control sr-control validate-field" name="odc_no" value="<?= htmlspecialchars($row['odc_no'] ?? '') ?>" placeholder="ODC No">
                                <div class="invalid-feedback">⚠️ Wajib diisi</div>
                            </div>
                            <div class="col-4">
                                <label class="sr-label">JC No <span class="text-danger">*</span></label>
                                <input class="form-control sr-control validate-field" name="jc_no" value="<?= htmlspecialchars($row['jc_no'] ?? '') ?>" placeholder="JC No">
                                <div class="invalid-feedback">⚠️ Wajib diisi</div>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control sr-control validate-field" name="odp" value="<?= htmlspecialchars($row['odp'] ?? '') ?>" placeholder="ODP">
                            <div class="invalid-feedback">⚠️ ODP wajib diisi</div>
                        </div>
                        <div class="form-group">
                            <label class="sr-label">ODC <span class="text-danger">*</span></label>
                            <input class="form-control sr-control validate-field" name="odc" value="<?= htmlspecialchars($row['odc'] ?? '') ?>" placeholder="ODC">
                            <div class="invalid-feedback">⚠️ ODC wajib diisi</div>
                        </div>
                        <div class="form-group">
                            <label class="sr-label">Enclosure <span class="text-danger">*</span></label>
                            <input class="form-control sr-control validate-field" name="enclosure" value="<?= htmlspecialchars($row['enclosure'] ?? '') ?>" placeholder="Enclosure">
                        ...
                        <div class="form-row mb-0">
                            <div class="col-6">
                                ...
                                <input class="form-control sr-control" name="mac_sebelum" value="<?= htmlspecialchars($row['mac_sebelum'] ?? '') ?>" placeholder="MAC sebelum">
                            </div>
                            <div class="col-6">
                                ...
                                <input class="form-control sr-control" name="mac_sesudah" value="<?= htmlspecialchars($row['mac_sesudah'] ?? '') ?>" placeholder="MAC sesudah">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card sr-card shadow-sm mb-4">
                    ...
                    <div class="card-body">
                        <?php if ($isTim): ?>
                            ...
                            <div class="pic-box">
                                <?php foreach ($teamMembers as $tm): ?>
                                    <?php $checked = in_array($tm['tech_id'], $currentPicIds, true) ? 'checked' : ''; ?>
                        ...
                        <?php elseif (!empty($currentPics) || !empty($teamMembers)): ?>
                            <?php
                            $picSource  = !empty($currentPics) ? $currentPics : $teamMembers;
                            $picNames   = array_filter(array_column($picSource, 'name'));
                            $picIdsOut  = array_column($picSource, 'tech_id');
                            ?>
                            <input class="form-control sr-control" readonly
                                value="<?= htmlspecialchars($picNames ? implode(', ', $picNames) : implode(', ', $picIdsOut)) ?>">
                            <?php foreach ($picIdsOut as $pid): ?>
                                <input type="hidden" name="pic[]" value="<?= htmlspecialchars($pid) ?>">
                            <?php endforeach; ?>
                        <?php else: ?>
                            <p class="text-muted mb-0" style="font-size:12px">Belum ada teknisi yang ditugaskan pada laporan ini.</p>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <button type="button" id="btnSubmit" onclick="submitAndShare()"
                class="btn btn-submit-main btn-block mb-4">
                💾 Update & Share
            </button>
...
<script>
    var TECHNICIAN_MAP = <?= json_encode(
                                array_reduce($technicianMapSource, function ($carry, $item) {
    ...
    async function submitAndShare() {
        ...
        // Kompatibilitas: controller kemungkinan cek isset($_POST['submit'])
        ...
        try {
            ...
            if (result.status) {
                ...
                Swal.fire({
                    icon: 'success',
                    title: 'Berhasil!',
                    text: 'Perubahan laporan berhasil disimpan.',
